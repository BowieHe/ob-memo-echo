...
import type { HealthCheckResult } from "@core/types/health-check";
...
export class MemoEchoSettingTab extends PluginSettingTab {
	...
	/**
	 * Helper method to add connection check button for a service
	 */
	private addConnectionCheck(
			...
			.addButton((button) =>
				button.setButtonText("检查连接").onClick(async () => {
					try {
						...
						if (result.connected) {
							new Notice(`✅ ${serviceName} ${result.message}`);
						} else {
							new Notice(`❌ ${serviceName} ${result.message}`);
						}
					} catch (error) {
						new Notice(
							`❌ ${serviceName} 检查失败: ${getErrorMessage(error)}`,
						);
					}
				}),
			);
	...
	/**
	 * Check all service connections at once
	 */
	private async checkAllConnections(): Promise<void> {
		const checks = [
			{
				name: "Qdrant",
				fn: () => this.plugin.vectorBackend.checkConnection(),
			},
			{
				name: "Embedding",
				fn: () => this.plugin.embeddingService.checkConnection(),
			},
			{
				name: "LLM",
				fn: () => this.plugin.metadataExtractor.checkConnection(),
			},
		];

		const results = await Promise.allSettled(
			checks.map((c) => c.fn().then((r) => ({ name: c.name, ...r }))),
		);

		results.forEach((result, index) => {
			if (result.status === "fulfilled") {
				const r: HealthCheckResult & { name: string } = result.value;
				const icon = r.connected ? "✅" : "❌";
				new Notice(`${icon} ${r.name}: ${r.message}`);
			} else {
				new Notice(`❌ ${checks[index].name}: 检查失败`);
			}
		});
	}
		...
		try {
			...
			for (let i = 0; i < files.length; i++) {
				...
				try {
					...
					await this.plugin.indexManager.updateFile(
						file.path,
						content,
					);
					indexed++;
	...
	private addServiceStatusSection(containerEl: HTMLElement): void {
		...
		// Qdrant Status
		// this.addConnectionCheck(
		//     statusContainer,
		//     'Qdrant',
		//     '向量数据库连接状态',
		//     async () => await this.plugin.vectorBackend.checkConnection()
		// );
		...
		//     'Embedding 服务',
		//     '向量生成服务连接状态',
		//     async () => await this.plugin.embeddingService.checkConnection()
		// );
		...
		//     'LLM 服务',
		//     'AI生成服务连接状态',
		//     async () => await this.plugin.metadataExtractor.checkConnection()
		// );

		// Check All Button
		new Setting(statusContainer)
			.setName("检查所有连接")
			.setDesc("同时检查所有服务连接状态")
			.addButton((button) =>
				button
					.setButtonText("检查所有")
					.setCta()
	...
	private addEmbeddingSection(containerEl: HTMLElement): void {
			...
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange(async (value) => {
						const result =
							await this.plugin.settingsManager.updateEmbedding({
								provider: value as "ollama" | "openai",
							});
						if (
							this.handleSettingsResult(
								result,
								`已切换到 ${value} 提供商`,
							)
						) {
		...
		if (config.provider === "ollama") {
			...
			embedModelSetting.addButton((button) =>
				button
					.setButtonText("刷新列表")
					.setTooltip("刷新模型列表")
					.onClick(() => {
		...
		if (config.provider === "openai") {
			...
			new Setting(containerEl)
				...
				.setDesc(
					"OpenAI-compatible 服务的 API Key (本地 Ollama 可留空)",
				)
				.addText((text) =>
					text
						.setPlaceholder("sk-...")
	...
	private addAiGenerationSection(containerEl: HTMLElement): void {
		...
		new Setting(group)
			...
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange(async (value) => {
						const result =
							await this.plugin.settingsManager.updateLlm({
								provider: value as "ollama" | "openai",
							});
						if (result.success) {
							this.display();
		...
		if (config.provider === "ollama") {
			...
			genModelSetting.addDropdown(async (dropdown) => {
				...
				dropdown.setValue(config.model).onChange(async (value) => {
					...
					const result = await this.plugin.settingsManager.updateLlm({
						model: value,
					});
					if (!result.success) {
						new Notice(
			...
			genModelSetting.addButton((button) =>
				button.setButtonText("刷新列表").onClick(() => this.display()),
			);
		}
		...
		if (config.provider === "openai") {
			...
			new Setting(group)
				...
				.setDesc(
					"OpenAI-compatible API 地址 (如 https://api.deepseek.com/v1)",
				)
				.addText((text) =>
					text
		...
		if (this.plugin.settings.enableConceptExtraction) {
			new Setting(conceptGroup)
				...
				.addToggle((toggle) =>
					toggle
						.setValue(
							this.plugin.settings.conceptFE.injectToFrontmatter,
						)
						.onChange(async (value) => {
							const result =
								await this.plugin.settingsManager.updateConceptFE(
									{ injectToFrontmatter: value },
								);
							if (result.success) {
								this.display();
							} else {
			...
			if (this.plugin.settings.conceptFE.injectToFrontmatter) {
				new Setting(conceptGroup)
					...
					.addToggle((toggle) =>
						toggle
							.setValue(
								this.plugin.settings.conceptFE
									.autoCreateConceptPage,
							)
							.onChange(async (value) => {
								const result =
									await this.plugin.settingsManager.updateConceptFE(
										{ autoCreateConceptPage: value },
									);
								if (!result.success) {
									new Notice(
										`❌ 更新失败: ${result.errors?.[0]?.message}`,
			...
			new Setting(conceptGroup)
				...
				.addText((text) =>
					text
						.setPlaceholder("_me")
						.setValue(
							this.plugin.settings.conceptFE.conceptPagePrefix,
						)
						.onChange(async (value) => {
			...
			const abstractGroup = containerEl.createDiv(
				"memo-echo-settings-group",
			);
			...
			new Setting(abstractGroup)
				...
				.addToggle((toggle) =>
					toggle
						.setValue(
							this.plugin.settings.conceptExtraction
								.focusOnAbstractConcepts,
						)
						.onChange(async (value) => {
							const result =
								await this.plugin.settingsManager.updateConceptExtraction(
									{ focusOnAbstractConcepts: value },
								);
							if (!result.success) {
								new Notice(
									`❌ 更新失败: ${result.errors?.[0]?.message}`,
			...
			new Setting(abstractGroup)
				...
				.addTextArea((text) =>
					text
						.setPlaceholder("技术开发\n总结\n概述")
						.setValue(
							this.plugin.settings.conceptExtraction.excludeGenericConcepts.join(
								"\n",
							),
						)
						.onChange(async (value) => {
							...
							const result =
								await this.plugin.settingsManager.updateConceptExtraction(
									{ excludeGenericConcepts: excludeList },
								);
							if (!result.success) {
								new Notice(
									`❌ 更新失败: ${result.errors?.[0]?.message}`,
			...
			new Setting(skipGroup)
				...
				.addTextArea((text) =>
					text
						.setValue(
							this.plugin.settings.conceptSkip.skipPaths.join(
								"\n",
							),
						)
						.onChange(async (value) => {
							const skipPaths = value
								.split("\n")
								.map((s) => s.trim())
								.filter((s) => s.length > 0);
							const result =
								await this.plugin.settingsManager.updateConceptSkip(
									{ skipPaths },
								);
							if (!result.success) {
								new Notice(
			...
			new Setting(skipGroup)
				...
				.addText((text) =>
					text
						.setValue(
							this.plugin.settings.conceptSkip.skipTags.join(
								", ",
							),
						)
						.onChange(async (value) => {
							const skipTags = value
								.split(",")
								.map((s) => s.trim())
								.filter((s) => s.length > 0);
							const result =
			...
			new Setting(skipGroup).setName("最小文本长度").addText((text) =>
				text
					.setValue(
						this.plugin.settings.conceptSkip.minTextLength.toString(),
					)
					.onChange(async (value) => {
						...
						.onClick(async () => {
							...
							if (confirmed) {
								try {
									new Notice("🔄 正在清除...");
									const result =
										await this.plugin.frontmatterService.clearAllMemoEchoFieldsFromAllFiles();
									new Notice(
										`✅ 已清除 ${result.cleared} 个文件${result.failed > 0 ? `, ${result.failed} 个失败` : ""}`,
									);
								} catch (error) {
									new Notice(
										`❌ 清除失败: ${getErrorMessage(error)}`,
									);
								}
							}
						}),
			...
			new Setting(abstractGroup)
				...
				.addButton((button) =>
					button.setButtonText("清除最近").onClick(async () => {
						try {
							const cutoffDate = new Date(
								Date.now() - 7 * 24 * 60 * 60 * 1000,
							);
							const recentFiles =
								await this.plugin.frontmatterService.getFilesIndexedAfter(
									cutoffDate,
								);
							...
							for (const file of recentFiles) {
								try {
									await this.plugin.frontmatterService.clearMemoEchoFields(
										file,
									);
									cleared++;
								} catch (e) {
