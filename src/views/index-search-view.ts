...
export class IndexSearchView extends ItemView {
	...
	async onOpen(): Promise<void> {
		...
		if (!container) {
			console.warn(
				"[IndexSearchView] Container element not found at children[1], creating fallback container",
			);
			this.container = this.containerEl.createDiv(
				"unified-search-view-container",
			);
		} else {
	...
	private renderReact() {
		try {
			if (!this.container) {
				console.error(
					"[IndexSearchView] Container is null, cannot mount React component",
				);
				return;
			}
			...
			console.error(
				"[IndexSearchView] ❌ Failed to mount React component:",
				error,
			);
		}
	}
	...
	async updateRecommendations(paragraph: string): Promise<void> {
		try {
			...
			window.dispatchEvent(
				new CustomEvent("memo-echo:ambient-update", {
					detail: results,
				}),
			);
		} catch (error) {
			console.error(
				"[IndexSearchView] Failed to get recommendations:",
				error,
			);
		}
	}
